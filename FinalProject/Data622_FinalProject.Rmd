...
pca <- prcomp(nums)
summary(pca)
plot(pca, type = 'l', col = 'light blue')
fviz_eig(pca)
fviz_contrib(pca, choice = "var", axes = c(1,2), top = 15)
fviz_pca_var(pca,
...
ideal_k <- knnreg(income ~., data = train_knn)
# Train KNN model
train.knn <- (train_knn[, names(train_knn) != "income"])
cl <- trainControl(method="repeatedcv", repeats = ideal_k$k) 
knn_model <- knn3(income ~ ., data = train_knn, k = ideal_k$k, use.all = F)
head(predict(knn_model, test_knn, type = "class"), n = 10)
...
prep
cl <- trainControl(method="repeatedcv", repeats = 3) 
knn_model <- train(income ~ ., data = training, 
                method = "knn", 
                trControl = cl, 
...
X_test <- sapply(subset(testing, select = -income), as.numeric)
```
